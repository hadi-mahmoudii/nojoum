...
class AudioPlayerHandler extends BaseAudioHandler with SeekHandler {
  ...
  AudioPlayerHandler(this._item, this._player) {
...
class MusicDetailsController extends GetxController {
  ...
  getPreviousMusic() {
    fetchData(previousMusics.first.id, true);
  }
  ...
  fetchData(String musicId, bool typeIsMus, {RadioModel? rad}) async {
    ...
    // if back to current playing music,we most ignore get datas(we most check to wont come from radio to last music played before radio)
    try {
      if (musicId == music.id && typeIsMus) {
        isLoading.value = false;
        return;
      }
    } catch (_) {}
        ...
        await audioPlayer.setAudioSource(
          AudioSource.uri(
            ...
            tag: MediaItem(
              ...
              // Metadata to display in the notification:
              title: rad.name,
              artUri: Uri.parse(rad.thumbnail),
            ),
          ),
        );
        // await audioPlayer.setUrl(rad.link);
        audioPlayer.playerStateStream.listen((playerState) {
          final isPlaying = playerState.playing;
          final processingState = playerState.processingState;
          if (processingState == ProcessingState.loading ||
  ...
  Future<List<OptionModel>> getUserPlaylists() async {
